...
The bulk lifecycle repair command was not run and is not proposed. All token findings below expose only usability/source classes; no BRIC, AUTH_GUID, or routing value was read into this report.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Michael Palomo, member 61, subscription 66 | `cancelled/false` -> unchanged | `active` -> `cancelled` | `automatic` -> `disabled` | `null` -> `member_cancelled` | `null` -> `2026-08-24T05:02:53.031Z` | `2026-09-15T07:25:22.150Z` -> unchanged | `missing_payment_credentials` -> unchanged | Preview immediate atomic lifecycle normalization using the recorded cancellation timestamp; owner explicitly confirms cancellation. |
| Rebeca Iglesias, member 53, subscription 58 | `cancelled/false` -> unchanged | `active` -> `cancelled` | `automatic` -> `disabled` | `null` -> `member_cancelled` | `null` -> `2026-07-11T01:28:09.976Z` | `2026-07-30T11:16:10.009Z` -> unchanged | `missing_payment_credentials` -> unchanged | Preview immediate atomic lifecycle normalization using the recorded cancellation timestamp; owner explicitly confirms cancellation. |
...
| Account | Member status / active | Subscription status | Billing mode | Pending reason | End date | Next billing | Token usability | Proposed action and reason |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Kalyan Dinavahi, member 15, subscription 15 | `cancelled/false` -> unchanged | `active` -> `cancelled` | `automatic` -> `disabled` | `member_cancelled` -> unchanged | `null` -> `2026-08-18T03:07:00.953Z` | `2026-08-03T06:00:00.000Z` -> unchanged | `usable:payment_tokens.original_network_trans_id` -> unchanged | Preview immediate atomic normalization; legacy reason is `Cancelled per member request`. |
| Daniel Aviles, member 24, subscription 24 | `cancelled/false` -> unchanged | `cancelled` -> unchanged | `automatic` -> `disabled` | `member_cancelled` -> unchanged | `null` -> `2026-09-02T03:44:08.967Z` | `2026-05-30T06:00:00.000Z` -> unchanged | `missing_payment_credentials` -> unchanged | Preview atomic normalization; legacy reason says member cancelled and was removed from recurring billing by the 2026-09-01 billing audit. |
| Mackenzy White, member 22, subscription 29 | `cancelled/false` -> unchanged | `cancelled` -> unchanged | `automatic` -> `disabled` | `null` -> `member_cancelled` | `2026-05-23T06:00:00.000Z` -> `2026-08-13T20:03:23.292Z` | `2026-05-25T06:00:00.000Z` -> unchanged | `usable:payment_tokens.original_network_trans_id` -> unchanged | Preview atomic normalization; legacy reason is `Cancelled per member request`. Replace the stale period boundary with the recorded cancellation-effective timestamp only if approved. |
| Joiniesha Bell, member 20, subscription 31 | `cancelled/false` -> unchanged | `cancelled` -> unchanged | `automatic` -> `disabled` | `null` -> `member_cancelled` | `2026-05-10T17:25:14.579Z` -> `2026-08-13T20:03:23.025Z` | `2026-06-23T06:00:00.000Z` -> unchanged | `usable:payment_tokens.original_network_trans_id` -> unchanged | Preview atomic normalization; legacy reason is `Cancelled per member request`. Replace the stale period boundary only if approved. |
...
| Account | Member status / active | Subscription status | Billing mode | Pending reason | End date | Next billing | Token usability | Proposed action and reason |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| James Meeks, member 19, subscription 19 | `cancelled/false` -> unchanged | `cancelled` -> unchanged | `automatic` -> `disabled` | `null` -> `member_cancelled` | `null` -> `2026-07-08T22:59:33.976Z` | `2026-08-18T06:00:00.000Z` -> unchanged | `usable:payment_tokens.original_network_trans_id` -> unchanged | Preview atomic normalization, contingent on owner confirmation of the legacy cancelled state. |
| Reginald Kennedy, member 25, subscription 25 | `cancelled/false` -> unchanged | `suspended` -> `cancelled` | `automatic` -> `disabled` | `payment_delinquent` -> `member_cancelled` | `null` -> `2026-07-11T22:37:36.484Z` | `2026-06-09T23:04:52.807Z` -> unchanged | `usable:payment_tokens.original_network_trans_id` -> unchanged | Preview atomic normalization, contingent on confirming cancellation rather than delinquency suspension. |
| Rodrigo Montelongo, member 30, subscription 36 | `cancelled/false` -> unchanged | `suspended` -> `cancelled` | `automatic` -> `disabled` | `payment_delinquent` -> `member_cancelled` | `null` -> `2026-07-11T22:37:24.111Z` | `2026-05-16T22:30:41.978Z` -> unchanged | `missing_or_invalid_processor_reference` -> unchanged | Preview atomic normalization, contingent on confirming cancellation rather than delinquency suspension. |
| Ayodele Adebayo, member 32, subscription 38 | `cancelled/false` -> unchanged | `pending_payment` -> `cancelled` | `automatic` -> `disabled` | `null` -> `member_cancelled` | `null` -> `2026-08-25T06:02:10.529Z` | `2026-05-01T06:00:00.000Z` -> unchanged | `missing_payment_credentials` -> unchanged | Preview atomic normalization, contingent on confirming cancellation of the pending-payment enrollment. |

## 2. Manual Or External

| Account | Member status / active | Subscription status | Billing mode | Pending reason | End date | Next billing | Token usability | Proposed action and reason |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Christian Parra, member 29, subscription 35 | `active/true` -> unchanged | `active` -> unchanged | `automatic` -> `manual_external` | `null` -> unchanged | `null` -> unchanged | `2026-09-07T20:45:42.186Z` -> unchanged | `missing_or_invalid_processor_reference` -> unchanged | Record external settlement evidence through the supervised admin workflow, then retain manual/external mode. Do not contact EPX. |
...
Payment 56 is linked to member 29 and subscription 35. It is already `succeeded` for `$123.76` and has a transaction ID, but `payment_confirmed_at`, `payment_transaction_at`, `platform_verified_at`, and `verification_method` are null. No usable recurring processor reference exists.
...
- `payment_confirmed_at`: `null` -> execution verification timestamp.
- `platform_verified_at`: `null` -> execution verification timestamp.
- `verification_method`: `null` -> `manual_admin`.
- `payment_transaction_at`: `null` -> remain `null` unless a trustworthy provider timestamp is supplied; never invent one.
- Payment metadata: add the supplied external method, reference, processed timestamp, authenticated operator, and `billingMode: manual_external`.
- Member status/active: `active/true` -> unchanged.
- Subscription status: `active` -> unchanged.
- Billing mode: `automatic` -> `manual_external`.
- Run `PaymentConfirmedService` idempotently for payment 56 after evidence is entered. That service does not contact EPX.
...
The field therefore originated as `currentPeriodEnd`/paid-through metadata for these rows, not cancellation evidence. Current reporting calls it access-through, and durable due selection excludes subscriptions after it. The new atomic cancellation function also writes cancellation effectiveness into the same field. Because the column now carries overloaded meanings, this preview proposes no change to the four active rows.
