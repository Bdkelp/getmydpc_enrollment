...
export const users = pgTable("users", {
  ...
  overrideCommissionRate: decimal("override_commission_rate", {
    precision: 5,
    scale: 2,
  }).default("0"),
...
export const agencyAgentAssignments = pgTable(
  "agency_agent_assignments",
  {
    id: serial("id").primaryKey(),
    agencyUserId: uuid("agency_user_id")
  ...
  (table) => [
    index("idx_agency_agent_assignments_agency_user_id").on(table.agencyUserId),
    index("idx_agency_agent_assignments_agent_user_id").on(table.agentUserId),
    index("idx_agency_agent_assignments_active").on(table.isActive),
    index("idx_agency_agent_assignments_pair").on(
      table.agencyUserId,
      table.agentUserId,
    ),
  ],
...
export const members = pgTable(
  "members",
  {
    ...
    customerNumber: varchar("customer_number", { length: 24 })
      .unique()
      .notNull(),
    memberPublicId: varchar("member_public_id", { length: 24 })
      .unique()
      .notNull(),
    ...
    totalMonthlyPrice: decimal("total_monthly_price", {
      precision: 10,
      scale: 2,
...
export const subscriptions = pgTable(
  ...
  (table) => [index("idx_subscriptions_member_id").on(table.memberId)],
);
...
export const payments = pgTable(
  ...
  (table) => [index("idx_payments_member_id").on(table.memberId)],
);
...
export const enrollmentModifications = pgTable("enrollment_modifications", {
  ...
  modifiedBy: varchar("modified_by")
    .references(() => users.id)
    .notNull(), // Agent or admin who made the change
...
export const groupMembers = pgTable(
  "group_members",
  {
    ...
    payorType: varchar("payor_type", { length: 32 }).notNull(),
    employerAmount: decimal("employer_amount", {
      precision: 10,
      scale: 2,
    }).default("0"),
    memberAmount: decimal("member_amount", { precision: 10, scale: 2 }).default(
      "0",
    ),
  ...
  (table) => [
    ...
    index("idx_group_members_household_base_number").on(
      table.householdBaseNumber,
    ),
    index("idx_group_members_household_member_number").on(
      table.householdMemberNumber,
    ),
...
export const paymentTokens = pgTable(
  "payment_tokens",
  {
    ...
    paymentMethodType: varchar("payment_method_type", { length: 20 }).default(
      "CreditCard",
    ...
    originalNetworkTransId: varchar("original_network_trans_id", {
      length: 255,
    }),
...
export const billingSchedule = pgTable(
  "billing_schedule",
  {
    id: serial("id").primaryKey(),
    memberId: integer("member_id")
    ...
    paymentTokenId: integer("payment_token_id")
      .references(() => paymentTokens.id, { onDelete: "restrict" })
      .notNull(),
...
export const recurringBillingLog = pgTable(
  "recurring_billing_log",
  {
    ...
    memberId: integer("member_id")
      .references(() => members.id, { onDelete: "cascade" })
      .notNull(),
    paymentTokenId: integer("payment_token_id").references(
      () => paymentTokens.id,
    ),
    billingScheduleId: integer("billing_schedule_id").references(
      () => billingSchedule.id,
    ),
...
export const usersRelations = relations(users, ({ many, one }: any) => ({
  ...
  agencyAssignments: many(agencyAgentAssignments),
  assignedToAgencies: many(agencyAgentAssignments),
  // commissions removed - use agent_commissions table
}));

export const agencyAgentAssignmentsRelations = relations(
  agencyAgentAssignments,
  ({ one }: any) => ({
    agencyUser: one(users, {
      fields: [agencyAgentAssignments.agencyUserId],
      references: [users.id],
    }),
    agentUser: one(users, {
      fields: [agencyAgentAssignments.agentUserId],
      references: [users.id],
    }),
    assignedByUser: one(users, {
      fields: [agencyAgentAssignments.assignedBy],
      references: [users.id],
    }),
    removedByUser: one(users, {
      fields: [agencyAgentAssignments.removedBy],
      references: [users.id],
    }),
  }),
);
...
export const familyMembersRelations = relations(
  ...
  ({ one }: any) => ({
    primaryUser: one(users, {
      fields: [familyMembers.primaryUserId],
      references: [users.id],
    }),
    primaryMember: one(members, {
      fields: [familyMembers.primaryMemberId],
      references: [members.id],
    }),
  }),
);
...
export const insertAgencyAgentAssignmentSchema = createInsertSchema(
  agencyAgentAssignments,
).omit({
  id: true,
  createdAt: true,
...
export const insertBillingScheduleSchema = createInsertSchema(
  billingSchedule,
).omit({
  id: true,
  createdAt: true,
...
export const insertRecurringBillingLogSchema = createInsertSchema(
  recurringBillingLog,
).omit({
  id: true,
  createdAt: true,
...
const allowSameDayPlanStart =
  ...
  process.env?.ENABLE_SAME_DAY_PLAN_START === "true";
...
export const registrationSchema = z.object({
  ...
  phone: z
    ...
    .refine(
      isValidUSAPhone,
      "Must be a valid USA phone number (10 or 11 digits)",
    ),
  dateOfBirth: z
    .string()
  ...
  planStartDate: z
    ...
    .refine(
      (value) =>
        isPlanStartDateAllowed(value, {
          includeSameDay: allowSameDayPlanStart,
        }),
  ...
  emergencyContactPhone: z
    ...
    .optional()
    .refine(
      (val) => !val || isValidUSAPhone(val),
      "Must be a valid USA phone number",
    ),
  // Plan selection
  ...
  privacyNoticeAcknowledged: z
    .boolean()
    .refine((val) => val === true, "Privacy notice must be acknowledged"),
});
...
export type {
  AgentCommission as Commission,
  InsertAgentCommission as InsertCommission,
} from "./clean-commission-schema";
export type InsertLead = z.infer<typeof insertLeadSchema>;
export type LeadActivity = typeof leadActivities.$inferSelect;
export type InsertLeadActivity = z.infer<typeof insertLeadActivitySchema>;
export type AgencyAgentAssignment = typeof agencyAgentAssignments.$inferSelect;
export type InsertAgencyAgentAssignment = z.infer<
  typeof insertAgencyAgentAssignmentSchema
>;
...
export type InsertRecurringBillingLog = z.infer<
  typeof insertRecurringBillingLogSchema
>;
