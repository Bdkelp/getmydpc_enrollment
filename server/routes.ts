...
// Enhanced Agent Stats with Filtering
router.get(
  "/api/agent/enhanced-stats",
  authenticateToken,
  async (req: AuthRequest, res) => {
    console.log("🔍 ENHANCED AGENT STATS ROUTE HIT - User:", req.user?.email, "Role:", req.user?.role);
    ...
    try {
        ...
        switch (period) {
          ...
          case 'custom':
            if (startDate && endDate) {
              ...
              dateParams = [new Date(startDate as string), new Date(endDate as string)];
            }
            break;
        }
      ...
      // Get member enrollments with date filtering
      const { data: enrollments } = await supabase
        .from('members')
        .select('*')
        .eq('enrolled_by_agent_id', agentId)
        .gte('created_at', dateParams[0]?.toISOString() || '1900-01-01')
        .lte('created_at', dateParams[1]?.toISOString() || '2100-01-01');

      // Get commission data with date filtering
      const { data: commissions } = await supabase
        .from('agent_commissions')
        .select('*')
        .eq('agent_id', agentId)
      ...
      const totalCommissions = commissions?.reduce((sum: number, c: any) => sum + (c.commission_amount || 0), 0) || 0;
      const paidCommissions = commissions?.filter((c: any) => c.payment_status === 'paid')
        .reduce((sum: number, c: any) => sum + (c.commission_amount || 0), 0) || 0;
      const pendingCommissions = totalCommissions - paidCommissions;

      // Calculate revenue (assuming average monthly fee per member)
      const averageMonthlyFee = 150; // This should come from plan data
      const totalRevenue = totalMembers * averageMonthlyFee;
      ...
      const yearlyEnrollments = enrollments?.filter(
        (e: any) => new Date(e.created_at) >= thisYear
      ).length || 0;
      ...
      const yearlyCommissions = commissions?.filter(
        (c: any) => new Date(c.created_at) >= thisYear
      ).reduce((sum: number, c: any) => sum + (c.commission_amount || 0), 0) || 0;

      const monthlyRevenue = monthlyEnrollments * averageMonthlyFee;
      const yearlyRevenue = yearlyEnrollments * averageMonthlyFee;

      // Growth calculations (simplified - would need historical data for real growth)
      const revenueGrowth = 0; // Placeholder
      const memberGrowth = 0; // Placeholder
      const commissionGrowth = 0; // Placeholder

      console.log("✅ Enhanced agent stats calculated for", req.user!.email);
      res.json({
        totalRevenue,
        monthlyRevenue,
        yearlyRevenue,
      ...
      console.error("❌ Error fetching enhanced agent stats:", error);
      res.status(500).json({ message: "Failed to fetch enhanced agent stats" });
    }
  },
);

// Enhanced Admin Stats with Filtering
router.get(
  "/api/admin/enhanced-stats",
  authenticateToken,
  async (req: AuthRequest, res) => {
    console.log("🔍 ENHANCED ADMIN STATS ROUTE HIT - User:", req.user?.email, "Role:", req.user?.role);
    ...
    try {
      ...
      // Build date filters (same logic as agent stats)
      let dateParams: any[] = [];
      ...
      // Get all member data with date filtering
      ...
      // Get all commission data with date filtering
      ...
      const activeMembers = allMembers?.filter((m: any) => m.is_active).length || 0;
      
      const totalCommissions = allCommissions?.reduce((sum: number, c: any) => sum + (c.commission_amount || 0), 0) || 0;
      const paidCommissions = allCommissions?.filter((c: any) => c.payment_status === 'paid')
        .reduce((sum: number, c: any) => sum + (c.commission_amount || 0), 0) || 0;
      const pendingCommissions = totalCommissions - paidCommissions;
      ...
      const monthlyEnrollments = allMembers?.filter(
        (m: any) => new Date(m.created_at) >= thisMonth
      ).length || 0;

      const yearlyEnrollments = allMembers?.filter(
        (m: any) => new Date(m.created_at) >= thisYear
      ).length || 0;

      const monthlyCommissions = allCommissions?.filter(
        (c: any) => new Date(c.created_at) >= thisMonth
      ).reduce((sum: number, c: any) => sum + (c.commission_amount || 0), 0) || 0;

      const yearlyCommissions = allCommissions?.filter(
        (c: any) => new Date(c.created_at) >= thisYear
      ).reduce((sum: number, c: any) => sum + (c.commission_amount || 0), 0) || 0;
      ...
      console.log("✅ Enhanced admin stats calculated for platform");
      res.json({
        totalRevenue,
        monthlyRevenue,
        yearlyRevenue,
      ...
      res.status(500).json({ message: "Failed to fetch enhanced admin stats" });
    }
  },
);
