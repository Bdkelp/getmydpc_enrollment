...
export async function registerRoutes(app: any) {
  ...
  app.post("/api/mock-payment", authMiddleware, async (req: any, res: any) => {
    try {
      ...
      // Don't pass 'id' field as it's auto-generated by the database
      const payment = await storage.createPayment({
        userId: userId,
        amount: String(amount || 79), // Ensure amount is a string
        currency: "USD", // Explicitly set currency to avoid schema cache issues
        status: "completed",
        transactionId: paymentId,
        paymentMethod: "test-card",
        authorizationCode:
          "TEST_AUTH_" + Math.random().toString(36).substr(2, 6),
        metadata: {
          environment: "test",
          testPayment: true,
      ...
      try {
        ...
        if (newStatus && (user.role === "member" || user.role === "user")) {
          try {
            ...
            console.log(
              "No subscription to reactivate or error reactivating:",
              error,
            );
          }
        } else if (user.role === "agent") {
          console.log(
            `Agent ${userId} status updated - no subscription handling needed`,
          );
        }
        ...
        for (const agent of agents) {
          if (!agent.isActive) {
            try {
              ...
              console.error(
                `Failed to reactivate agent ${agent.email}:`,
                error,
              );
            }
          }
