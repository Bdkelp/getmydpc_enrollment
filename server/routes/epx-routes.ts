...
// Initialize EPX Service on module load
try {
...
router.post("/api/epx/create-payment", async (req: Request, res: Response) => {
  try {
    ...
    console.log('[EPX Create Payment] ============ COMPLETE FORM DATA ============');
    console.log('[EPX Create Payment] Action URL:', formData.actionUrl);
    console.log('[EPX Create Payment] TAC:', tacResponse.tac);
    console.log('[EPX Create Payment] Complete Form Fields:', JSON.stringify({
      ...formData,
      TAC: tacResponse.tac.substring(0, 10) + '...' // Mask TAC for security
    }, null, 2));
    console.log('[EPX Create Payment] Field Count:', Object.keys(formData).length);
    console.log('[EPX Create Payment] ================================================');

    return res.json({
      success: true,
      formData,
      epxEndpoint: (process.env.EPX_ENVIRONMENT || "sandbox") === "sandbox"
...
/**
 * Debug Form Data Endpoint
 * Shows what form data would be generated (for debugging)
 */
router.get("/api/epx/debug-form", async (req: Request, res: Response) => {
  try {
    const epxService = getEPXService();
    
    // Generate sample form data
    const sampleFormData = epxService.getPaymentFormData(
      "SAMPLE_TAC_123456",
      100.00,
      "TEST123456",
      "card",
    ...
    return res.json({
      success: true,
      message: "Sample form data (what would be sent to EPX)",
      formData: sampleFormData,
      fieldCount: Object.keys(sampleFormData).length,
      fields: Object.keys(sampleFormData),
    });
  } catch (error: any) {
    return res.status(500).json({
      success: false,
  ...
  try {
    ...
    return res.json({
      ...
      endpoints: {
        ...
        browserPost: environment === "sandbox"
          ? "https://services.epxuap.com/browserpost/"
          : "https://services.epx.com/browserpost/",
      },
    });
