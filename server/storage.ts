...
const mapEnrollmentRowToDetails = (row: EnrollmentRow, familyRows: FamilyMemberRow[]) => {
  const mapped = {
    ...
    enrollmentDate: (row as any).enrollment_date || null,
    firstPaymentDate: (row as any).first_payment_date || null,
    membershipStartDate: (row as any).membership_start_date || null,
    planId: row.plan_id,
